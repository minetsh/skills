...
6. 目视检查生成源图；若不理想，按“定向重试策略”朝固定方向补充描述后重试，而非随机更换措辞。
7. 将选定的生成源图复制到工作目录或任务输出目录。
8. 运行 `scripts/prepare_sticker.py` 去除抠像、缩放到目标尺寸、按需补强白色描边并写出校验 JSON。
9. 目视检查最终 PNG：主体不裁切、文字清晰且不遮脸、四角透明、尺寸精确。
10. 如需封面图/聊天图标/横幅，按上述规范单独产出（封面/图标关闭描边、调整尺寸），并各自使用“分素材提示词 brief”一节的专属模板扩写提示词。
11. 生成 `表情合集.png`：把本次表情图整理成统一风格的合集预览图，便于快速浏览整套表情（使用“分素材提示词 brief”中的合集模板）。
12. 生成 `宣传海报.png`：围绕本套表情主题制作宣传海报图，突出角色、表情名称与风格卖点（使用“分素材提示词 brief”中的海报模板）。
13. 按“最终输出目录与命名”整理最终交付文件。
14. 根据表情主题与规范自动生成提交文案，写入 `preview.json`，运行 `scripts/build_preview.py` 生成提交预览页 `预览.html`（见“提交预览页”一节）。
15. 报告最终路径、生成方式、校验结果与 `预览.html` 路径。
...
- 背景去不干净 → 强化纯色抠像约束，必要时换更干净抠像色并调 `prepare_sticker.py` 阈值。
