...
- 禁止 emoji 图标（铁律）：**任何生成的图片（表情图、封面、图标、横幅、赞赏图、合集、海报等）一律不得出现 emoji/绘文字符号或系统表情图标**（如 😀🐶❤️✨ 等），主体与情绪须由插画本身的造型与神态表达，而非贴 emoji。所有生成场景的提示词都须在 `Avoid:` 中加入 `no emoji, no emoji icons, no emoji-style glyphs, no system emoji`。
