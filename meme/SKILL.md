...
- **宣传海报（`宣传海报.png`）**：围绕主题的营销海报，突出角色与风格卖点，构图有主次与层次；可含表情名称等少量文字（与微信平台必填素材的“无文字”要求无关，此为对外展示物）。套用「海报/banner 类高级设计基调」并取用下文「高级海报锚点」抬升质感，但**务必以本套角色/主题/主色为绝对主体融合，锚点只作风格修饰，不可套脱离主题的通用模板**；图上文字（如表情名称）按「赞赏图文字设计要点」同理设计化融入，不要平贴字幕。
